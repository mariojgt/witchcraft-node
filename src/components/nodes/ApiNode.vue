...
<template>
    <div
        class="bg-gray-900/95 backdrop-blur border-[1px] border-cyan-500/30 rounded-xl p-4 min-w-[300px] relative text-gray-100">
        <!-- Header -->
        ...
        <div class="space-y-4">
            ...
            <div class="space-y-2">
                <label class="text-xs uppercase tracking-wider text-cyan-400">Endpoint URL</label>
                <input v-model="data.url" placeholder="https://api.example.com/endpoint"
                    class="w-full bg-gray-800 border-0 rounded-lg focus:ring-2 focus:ring-cyan-500 text-gray-100" />
            </div>

            <div class="space-y-2">
                <label class="text-xs uppercase tracking-wider text-cyan-400">Request Body</label>
                <textarea v-model="data.body" placeholder="JSON payload" rows="3"
                    class="w-full bg-gray-800 border-0 rounded-lg focus:ring-2 focus:ring-cyan-500 text-gray-100 resize-none"></textarea>
            </div>

            <div class="space-y-2">
                <label class="text-xs uppercase tracking-wider text-cyan-400">Headers</label>
                <textarea v-model="data.headers" placeholder="JSON headers" rows="2"
                    class="w-full bg-gray-800 border-0 rounded-lg focus:ring-2 focus:ring-cyan-500 text-gray-100 resize-none"></textarea>
            </div>

            <label class="flex items-center gap-2 text-cyan-400 cursor-pointer">
                <input type="checkbox" v-model="data.saveResponse"
        ...
        <div
            class="absolute top-1/2 -right-3 w-3 h-6 bg-gray-800 rounded-r-lg border-r-2 border-y-2 border-cyan-500/50">
